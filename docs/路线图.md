...
> **⚠️ 目标变更（2026-06-25）**：本产品不再追求规模化用户，**定位 = 作品集 + 简历展示 + 朋友体验，低成本优先**。
...
> - Expo App **留作跨端能力展示**：能跑 + 网页扫码下载 APK（展示包即可），不上架、不做生产化
> - `packages/core`：保留（架构能力加分点）
>
> **不做（纯生产化、对目标无用）**
> - ❌ Apple App Store（$99/年，当前不投入）
> - ❌ EAS 正式出包 / APK 分发基建（展示包给朋友体验即可）
